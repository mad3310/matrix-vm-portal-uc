...
(function($){
	...
	return this.each(function(num){
		...
		this.limit=function(limit){
			if(body.length<=limit||limit=='all'){
				...
				if(!opt.morefn.status){
					...
					var showbody=body.substring(0,limit)+opt.fill+getbuttom(opt.morefn.moretext);
				}
			}
			$this.html(showbody);
		};
		//fangqi 2012-4-9 add start
		...
		//fangqi 2012-4-9 add end
		this.init(opt.limit);		
		$("."+opt.morefn.cssclass+"_"+num).live("click",function(){
			if($(this).html()==opt.morefn.moretext){
				showbody=body+getbuttom(opt.morefn.lesstext);
				$this.html(showbody);
				opt.morefn.fullfn();
			}else{
